...
def replace_affiliate_links(html, cfg):
    ...
    def _sub(match):
        ...
        return f'href="{target_url}" target="_blank" rel="nofollow noopener"'

    pattern = re.compile(r'href="(?P<scheme>ozon|wb)://(?P<target>[^"]+)"')
    return pattern.sub(_sub, html)


WB_BUTTON_RE = re.compile(r'\s*<a[^>]*href="wb://[^"]*"[^>]*>.*?</a>')
...
def render_article(fm, body_md, cfg):
    ...
    html_body = WB_BUTTON_RE.sub("", html_body)
    html_body = replace_affiliate_links(html_body, cfg)
    return html_body
